...
def getTotalUsedDiskSpace(directory):
    # goes through and calculates sizes for directories recursively
    size = 0

    for child in directory.children:
        if child.is_directory:
            size += getTotalUsedDiskSpace(child) 
        else:
            size += child.size

    return size

def getDeletionCandidates(directory):
    ...
    for child in directory.children:
        if child.is_directory:
            size += getDeletionCandidates(child) 
        else:
            size += child.size

    # adds deletion candidates to list
    if size >= target:
...
deletion_candidates = []
target = 30000000 - (70000000 - getTotalUsedDiskSpace(root))
getDeletionCandidates(root)
size_of_deletion = min(deletion_candidates)
print("Total size of the directory to be deleted: " + str(size_of_deletion))
